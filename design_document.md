...
### 6.4 分析機能 (Analytics Dashboard) - New!
-   **コンセプト**: データの視覚化により、ユーザーのモチベーション維持を支援します。
-   **主な機能**:
    -   **サマリー表示**: 総達成率、継続中のストリーク数、応援メッセージ（アドバイス）。
    -   **週間達成推移**:
        -   過去4週間（今週を含む）の達成タスク数をラインチャートで表示。
        -   画面の見やすさを考慮し、グラフはコンパクトなサイズ (高さ 200px) に調整。
        -   *変更点*: 当初予定していた「月間/週間切り替え」は廃止し、より直近の行動変容にフォーカスするため「週間のみ」のシンプル表示を採用。
    -   **曜日別傾向**:
        -   曜日ごとの累積達成数をバーチャートで表示し、自分の得意な曜日を可視化。
    -   **多言語対応**: UI内のすべてのテキストを日本語化。
    -   **デザイン**:
        -   プレミアムなGlassmorphismデザインを採用。
        -   レスポンシブ対応により、1画面に全ての重要な情報が収まるよう設計。

## 7. UI/UX デザイン方針
-   **ダークモード**: ベースカラー `#0f172a` を基調。
-   **グラスモーフィズム** & **レスポンシブ**: モバイルファーストな設計思想を取り入れ、スマホでもPCでも快適に操作可能。
-   **プレミアム感の演出**:
    -   チャートやトグルボタン（廃止済みだがコンポーネントとしては残存）にリッチなグラデーションや光沢エフェクトを使用。
    -   マイクロアニメーションによる心地よい操作感。
